...
# Optional external summarizer key pool. Set DEEPSEEK_SUMMARIZER_API_KEYS as a
# comma-separated list when using a hosted OpenAI-compatible summarizer.
DEEPSEEK_SUMMARIZER_API_KEYS = _env_list("DEEPSEEK_SUMMARIZER_API_KEYS")
DEEPSEEK_SUMMARIZER_BASE_URL = os.environ.get("DEEPSEEK_SUMMARIZER_BASE_URL", "")


# Optional Google Serper key pool. SERPER_API_KEY is also accepted for a single key.
SERPER_API_KEYS = _env_list("SERPER_API_KEYS")
if not SERPER_API_KEYS and os.environ.get("SERPER_API_KEY"):
    SERPER_API_KEYS = [os.environ["SERPER_API_KEY"]]
_serper_key_index = 0  # index of currently active key


def _current_serper_key(fallback: str = "") -> str:
    keys = SERPER_API_KEYS or ([fallback] if fallback else [])
    if not keys:
        raise FatalAPIError("SERPER_API_KEY or SERPER_API_KEYS is required for Google search.")
    return keys[min(_serper_key_index, len(keys) - 1)]
...
def _rotate_serper_key() -> bool:
    ...
    if _serper_key_index + 1 >= key_count:
        print(f"[SERPER KEY] All {key_count} keys exhausted!", flush=True)
        return False
    _serper_key_index += 1
    print(f"[SERPER KEY] Rotated to key index {_serper_key_index}", flush=True)
    return True
...
def get_judge_client(judge_client_type: str, judge_base_url: str, judge_api_key: str,use_openai=False):
    ...
    if use_openai:
        openai_api_key = judge_api_key or os.environ.get("OPENAI_API_KEY", "")
        openai_api_base = judge_base_url or os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1")
...
# Global async judge client singletons (avoid recreating on every call)
_async_judge_client_local = None
_async_judge_client_openai = None

def get_async_judge_client(use_openai: bool = False):
    """Return a reusable AsyncOpenAI client for judge (singleton per type)."""
    from openai import AsyncOpenAI
    global _async_judge_client_local, _async_judge_client_openai

    if use_openai:
        if _async_judge_client_openai is None:
            openai_api_base = os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1")
            _async_judge_client_openai = AsyncOpenAI(
                api_key=os.environ.get("OPENAI_API_KEY", ""),
                base_url=openai_api_base,
            )
        return _async_judge_client_openai
    else:
        if _async_judge_client_local is None:
            judge_base_url = (
                os.environ.get("JUDGE_BASE_URL")
                or os.environ.get("judge_base_url")
                or "http://localhost:8181/v1"
            )
            _async_judge_client_local = AsyncOpenAI(
                api_key="EMPTY",
                base_url=judge_base_url,
            )
        return _async_judge_client_local
...
async def llm_judge_score(question: str, model_answer: str, ground_truth: list | str, image_path: str, judge_client: str, judge_base_url: str, judge_api_key: str, judge_temperature: float = 0.0) -> float:
    ...
    use_openai_judge = os.environ.get("USE_OPENAI_JUDGE", os.environ.get("use_openai_judge", "false")).lower() == "true"
    if use_openai_judge:
        print(f"use GPT-4o-2024-11-20")
        model = 'GPT-4o-2024-11-20'
    else:
        print(f"use Qwen3-VL-32B-Instruct judge")
        model =    "Qwen3-VL-32B-Instruct"


    # client = get_judge_client(judge_client, judge_base_url, judge_api_key,use_openai=use_openai_judge)
    # Use async client (singleton) to avoid blocking the event loop
    async_client = get_async_judge_client(use_openai=use_openai_judge)
    ...
    for gt in ground_truth:
        ...
        try:
            # Async call — does NOT block the event loop, other samples can proceed
...
def load_datasets(config_path: str, data_root: str = "") -> tuple[list[dict], dict]:
    ...
    for dataset_name, ds_config in config.items():
        ...
        print(f"Loading dataset: {dataset_name} from {annotation}...")

        # Normalize both JSON arrays and JSONL files into JSONL-style strings.
        lines_to_process = []
        ...
        if not content_lines:
            print(f"Warning: File {annotation} is empty.")
            continue

        first_line = content_lines[0]

        if first_line.startswith('['):
            print("  Detected Standard JSON format (list). Converting to JSONL in memory...")
            try:
                full_json_str = "\n".join(content_lines)
                ...
                for item in data_list:
                    lines_to_process.append(json.dumps(item, ensure_ascii=False))
                    
                print(f"  Converted {len(lines_to_process)} items from JSON list.")
            except json.JSONDecodeError as e:
                print(f"Error parsing JSON list in {annotation}: {e}")
                continue
        else:
        ...
        for idx, line in enumerate(lines_to_process):
            ...
            if "image_search_title_list" in raw:
                ...
                if isinstance(titles, list):
                    image_search_kwargs["image_search_title_list"] = titles[:IMAGE_SEARCH_MAX_RESULTS]
            if "image_search_thumbnail_list" in raw:
                thumbs = raw["image_search_thumbnail_list"]
                if isinstance(thumbs, list):
                    image_search_kwargs["image_search_thumbnail_list"] = thumbs[:IMAGE_SEARCH_MAX_RESULTS]
...
# NOTE: Lock/Semaphore must be created lazily inside the running event loop.
# Creating them at module level causes "Event loop is closed" errors in Python 3.10+.
_browser_init_lock = None  # asyncio.Lock, created on first use
_page_semaphore = None     # asyncio.Semaphore(30), created on first use


async def _get_browser_context():
    """Get or create a shared browser context."""
    global _playwright, _browser, _browser_context, _browser_init_lock, _page_semaphore
    # Lazily create primitives inside the running event loop
    if _browser_init_lock is None:
        _browser_init_lock = asyncio.Lock()
    if _page_semaphore is None:
        _page_semaphore = asyncio.Semaphore(30)
    if _browser_context is None:
        async with _browser_init_lock:
            if _browser_context is None:  # double-checked locking
                # Read proxy from environment (http_proxy / HTTP_PROXY / https_proxy / HTTPS_PROXY)
                _proxy_url = (os.environ.get("http_proxy") or os.environ.get("HTTP_PROXY")
                              or os.environ.get("https_proxy") or os.environ.get("HTTPS_PROXY"))
                _launch_kwargs = {"headless": True}
                if _proxy_url:
                    _launch_kwargs["proxy"] = {"server": _proxy_url}
                _playwright = await async_playwright().start()
                _browser = await _playwright.chromium.launch(**_launch_kwargs)
...
# A standalone requests-based fetcher is provided in url_fetcher.py for debugging
# or deployments that cannot run Playwright.
def _fetch_url_with_requests(url: str, timeout_sec: int = 15) -> Optional[str]:
    """Fallback: fetch URL with requests (no JS rendering)."""
    import requests
    ...
    resp = requests.get(url, headers=headers, proxies=proxies, timeout=timeout_sec, allow_redirects=True)
...
async def fetch_url_content(url: str, timeout_sec: int = 20) -> Optional[str]:
    ...
    Falls back to requests on Playwright failure.
    ...
    try:
        # _get_browser_context() also lazily initializes _page_semaphore
        ...
        async with _page_semaphore:
            ...
            try:
                ...
                playwright_error = f"Timeout fetching {url}"
                response = None

            if response is None and playwright_error is None:
                playwright_error = f"No response from {url}"

            if response is not None and response.status >= 400:
                playwright_error = f"HTTP {response.status} from {url}"
                response = None
            ...
            if response is not None:
                ...
                try:
                    ...
                    playwright_error = f"Content timeout for {url}"
                    html = None
                ...
                if html:
                    ...
                    if raw_content:
                        return raw_content
                    playwright_error = f"Empty content from {url}"
    ...
    # Playwright failed — try requests as fallback
    if playwright_error is not None:
        try:
            content = await asyncio.to_thread(_fetch_url_with_requests, url, timeout_sec)
            if content:
                return content
        except Exception:
            pass
        raise URLFetchError(f"Error fetching {url}: {playwright_error}")
...
async def summarize_content(
    ...
    # Determine if using deepseek API (use key pool for higher concurrency)
    use_deepseek = (
        "deepseek" in summarizer_model.lower()
        or bool(DEEPSEEK_SUMMARIZER_BASE_URL and summarizer_base_url.rstrip('/') == DEEPSEEK_SUMMARIZER_BASE_URL.rstrip('/'))
    )
    ...
    for attempt in range(max_retries):
        ...
        if use_deepseek:
            api_key = random.choice(DEEPSEEK_SUMMARIZER_API_KEYS) if DEEPSEEK_SUMMARIZER_API_KEYS else os.environ.get("DEEPSEEK_API_KEY", "")
            effective_base_url = DEEPSEEK_SUMMARIZER_BASE_URL or summarizer_base_url
        else:
            api_key = ""
            effective_base_url = summarizer_base_url
...
async def call_text_search(
    ...
    # serper_api_key='e419ba0249c4be0475a4439b2bb8da98afc51e11'
    headers = {"X-API-KEY": _current_serper_key(serper_api_key), "Content-Type": "application/json"}
    ...
    use_google = os.environ.get("USE_GOOGLE", os.environ.get("use_google", "true")).lower() == "true"
    engine = "google" if use_google else "meituan"

    # Check query cache (raw search results)
    if query_cache:
        cached_results = query_cache.get(query, engine, top_k)
        if cached_results is not None:
            search_results = cached_results
            all_search_results = cached_results

    if search_results is None and use_google:
        print(f"use google search")
        for attempt in range(max_serper_attempts):
            async with serper_semaphore:
                try:
                    headers["X-API-KEY"] = _current_serper_key(serper_api_key)

                    async with aiohttp.ClientSession() as session:
                        async with session.post(url, headers=headers, json=payload,proxy=proxy_url) as resp:
                            if resp.status != 200:
                                error_text = await resp.text()
                                is_quota_exhausted = resp.status in (429, 402) or (
                                    resp.status == 400 and "credits" in error_text.lower()
                                )
                                if is_quota_exhausted:
                                    # Quota exhausted - rotate to next key and retry immediately
                                    print(f"[SERPER KEY] HTTP {resp.status} quota exhausted, rotating key...", flush=True)
                                    if _rotate_serper_key():
                                        continue  # retry with new key
                                    else:
                                        raise Exception(f"HTTP {resp.status}: all Serper keys exhausted")
                            ...
                            if query_cache:
                                await query_cache.set(query, engine, all_search_results)
        ...
        from mt_text_search import mt_search_async
        for attempt in range(max_serper_attempts):
            try:
                search_results=await mt_search_async(query=query)
                break
            except Exception as e:
                print(f"[mt_text_search RETRY] Attempt {attempt + 1}/{max_serper_attempts} failed: {e}, ...", flush=True)
        all_search_results=search_results
        # Store in query cache
        if query_cache and search_results is not None:
            await query_cache.set(query, engine, search_results)
    ...
    async def fetch_and_summarize(item, max_retries: int = 3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # print(f"_web_fetch_stats.record_success()")
    ...
    if os.environ.get('no_fetch', 'false').lower() == 'true':
        summaries = []
    else:
        # print(f"fetch_and_summarize!!!")
        tasks = [fetch_and_summarize(item) for item in search_results]
        summaries = await asyncio.gather(*tasks, return_exceptions=True)
        summaries = [s if not isinstance(s, Exception) else None for s in summaries]
    ...
    if os.environ.get('no_summary', 'false').lower() == 'true':
        print(f"no summary")
        result=all_content
        ...
        if tool_call:
            ...
            if tool_name == "image_zoom_in_tool":
                ...
                if query and serper_api_key and summarizer_base_url and summarizer_model:
                    ...
                    query_cache = kwargs.get("query_cache")
...
def main():
    ...
    parser.add_argument("--exp-name", type=str, default="", help="Experiment name prefix for output directory (e.g. exp1 -> exp1_evalname)")
    ...
    parser.add_argument("--query-cache-dir", type=str, default="", help="Directory for raw query result cache (optional, caches raw search results without summarization)")
    ...
    # Prepend exp_name prefix if provided
    if args.exp_name:
        parent = os.path.dirname(output_dir)
        basename = os.path.basename(output_dir)
        output_dir = os.path.join(parent, f"{args.exp_name}_{basename}") if parent else f"{args.exp_name}_{basename}"
    # Create search cache if directory provided
    search_cache = None
    if args.search_cache_dir:
        search_cache = SearchCache(args.search_cache_dir)

    # Create query cache (raw search results, no summarization)
    query_cache = None
    if args.query_cache_dir:
        from query_cache import QueryCache
        query_cache = QueryCache(args.query_cache_dir)
    ...
    async def health_check():
        ...
        if args.mode == "tool" and summarizer_base_url:
            ...
            is_external_api = (
                bool(DEEPSEEK_SUMMARIZER_BASE_URL and summarizer_base_url.rstrip("/") == DEEPSEEK_SUMMARIZER_BASE_URL.rstrip("/"))
                or "deepseek" in summarizer_base_url.lower()
            )
            if is_external_api:
                print(f"    Summarizer server skipped (external API, no /v1/models endpoint)", flush=True)
            else:
                try:
                    async with aiohttp.ClientSession(timeout=timeout) as session:
    ...
    try:
        ...
        if query_cache:
            ...
            print(f"Query cache: {stats['hits']}/{stats['total']} hits ({stats['hit_rate']:.1f}%), {stats['misses']} new queries cached", flush=True)
            query_cache.close()
